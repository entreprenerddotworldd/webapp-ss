...
const CONFIG_FILENAME = "webapp-ss.config.cjs";
const CONFIG_FILENAME_LEGACY = "webapp-ss.config.js";
...
function findConfigPath(projectDir) {
  const cjsPath = path.join(projectDir, CONFIG_FILENAME);
  if (fs.existsSync(cjsPath)) return cjsPath;
  const jsPath = path.join(projectDir, CONFIG_FILENAME_LEGACY);
...
function findProjectRoot(startDir) {
  ...
  while (dir !== path.dirname(dir)) {
    if (
      ...
      fs.existsSync(path.join(dir, CONFIG_FILENAME_LEGACY))
    ) {
      return dir;
    }
...
function loadConfig(projectDir) {
  ...
  try {
    ...
    console.error(`[!] Error loading ${path.basename(configPath)}: ${err.message}`);
...
function generateConfigTemplate() {
  return `// webapp-ss.config.cjs
// Web Application Security Scanner Configuration
// Uses .cjs extension to work in both ESM ("type": "module") and CommonJS projects
